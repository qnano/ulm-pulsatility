...
if ismember(mode,{'2D_vel_z','2D_velnorm','2D_velmean', '2D_theta'}),MatOut_vel = zeros(sizeOut); MatOut_vel_distr= cell(sizeOut);end
...
%         disp(['Building 2D ULM image loc (size : ' num2str(sizeOut) ').'])
        for itrack = 1:numel(Tracks)
            % round pixel position into [z,x] index
            pos_z_round = ceil(Tracks{itrack}(:,1));
            pos_x_round = ceil(Tracks{itrack}(:,2));
...
%         disp(['Building 2D ULM image vel (size : ' num2str(sizeOut) ').'])
        for itrack = 1:numel(Tracks)
            % round pixel position into [z,x] index
            pos_z_round = ceil(Tracks{itrack}(:,1));
            pos_x_round = ceil(Tracks{itrack}(:,2));
            ...
            if strcmp(mode,'2D_velmean')
                ...
                %velnorm = vecnorm(Tracks{itrack}(:,3:4),2,2);
            ...
            for j=1:size(ind,1)
                ...
                MatOut_vel_distr{ind(j)} = [MatOut_vel_distr{ind(j)},velnorm(j)];
            end
        end
    ...
    case {'2D_theta'}
        ...
        % theta defined as the angle between the x-axis in CCW direction
%         disp(['Building 2D ULM theta image (size : ' num2str(sizeOut) ').'])
        for itrack = 1:numel(Tracks)
            % round pixel position into [z,x] index
            pos_z_round = ceil(Tracks{itrack}(:,1));
            pos_x_round = ceil(Tracks{itrack}(:,2));
...
%             theta = atan(-Tracks{itrack}(:,3)./Tracks{itrack}(:,4));
%             for ii = 1:size(Tracks{itrack},1)
%                 if Tracks{itrack}(ii,4)<0
...
%             end
            for ii = 1:size(Tracks{itrack},1)
                if (Tracks{itrack}(ii,3) == 0 ) && (Tracks{itrack}(ii,4) == 0 )     % velz=>0 velx=>0
                    disp('ZERO ZERO')
                    if ii == 1
                        theta(ii)=0;
                    else
                ...
                elseif (Tracks{itrack}(ii,3) >= 0 ) && (Tracks{itrack}(ii,4) >= 0 )     % velz=>0 velx=>0
                    theta(ii)=2*pi - atan(Tracks{itrack}(ii,3)/Tracks{itrack}(ii,4));
                elseif (Tracks{itrack}(ii,3) <= 0 ) && (Tracks{itrack}(ii,4) >= 0 )     % velz<=0 velx=>0
                    theta(ii)=atan(-Tracks{itrack}(ii,3)/Tracks{itrack}(ii,4));
                elseif (Tracks{itrack}(ii,3) <= 0 ) && (Tracks{itrack}(ii,4) <= 0 )     % velz<=0 velx<=0
                    theta(ii)=pi - atan(Tracks{itrack}(ii,3)/Tracks{itrack}(ii,4));
                elseif (Tracks{itrack}(ii,3) >= 0 ) && (Tracks{itrack}(ii,4) <= 0 )     % velz>=0 velx<=0
                    theta(ii)=pi + atan(Tracks{itrack}(ii,3)/-Tracks{itrack}(ii,4));
                else
                    error('No angle case choosen')
                end
                if (theta(ii)>=0) && (theta(ii)<pi)
                    theta(ii)=theta(ii)+pi;
                elseif (theta(ii)>=pi) && (theta(ii)<= 2*pi+1E6)
                    theta(ii) = theta(ii)-pi;
                else
                    disp(theta(ii))
                    disp(ii)
                    disp(Tracks{itrack}(ii,3:4))
                    error('Theta error')
                    
                end
            end
            % smoothing
            velnorm = smooth(theta,10);
%             if velnorm(ii)>=2*pi
%                 velnorm(ii)=velnorm(ii)-2*pi;
%             elseif velnorm(ii)<0
%                 velnorm(ii)=velnorm(ii)+2*pi;
%             end 
...
%         varargout{1} = MatOut_vel;
        if exist('MatOut_vel_distr')
            varargout{1} = MatOut_vel_distr;
        end
        MatOut = MatOut_vel;
    else
        MatOut = MatOut_vel;
